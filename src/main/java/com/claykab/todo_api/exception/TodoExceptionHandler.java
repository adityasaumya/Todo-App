...
public class TodoExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<TodoCustomException> handleValidationExceptions(MethodArgumentNotValidException ex) {
        List<FieldError> fieldErrors = ex.getBindingResult().getFieldErrors();
        Map<String, String> errors = fieldErrors.stream()
            .collect(Collectors.toMap(FieldError::getField, FieldError::getDefaultMessage));
        TodoCustomException customException = new TodoCustomException(HttpStatus.BAD_REQUEST, "Validation failed", errors);
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
        ...
        return objectResponseEntity(new TodoCustomException(HttpStatus.BAD_REQUEST));
    }
